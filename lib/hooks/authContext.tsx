...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (username: string, password: string, rememberMe: boolean): Promise<UserDto> => {
    ...
    try {
      ...
      if (rememberMe) {
        ...
        // No Remember Me: Store only access token in sessionStorage
        // no refresh token
      ...
      // Always update React state to reflect logged-in user
  ...
  const logout = () => {
    ...
    // Clear auth state - user is now anonymous
    setToken(null);
    setUser(null);
  };
...
// Custom hook: Provides easy access to auth context from any component
export function useAuth() {
  const context = useContext(AuthContext);
  
  // Ensure hook is used correctly within AuthProvider
  if (!context) {
    throw new Error("useAuth must be used within AuthProvider");
  }
  
  return context;  // Return auth state and methods for use in component
}
